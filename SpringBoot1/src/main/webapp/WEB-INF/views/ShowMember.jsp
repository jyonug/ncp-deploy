				...
				<!-- Menu -->
					<nav id="Update">	
						<table>
							...
							<c:forEach items="${list}" var="m"> <!-- list에 있는 정보를 m이라는 변수에 담겠다 -->
							<tr>
								<td>${m.email}</td> <!-- m안에서 email을 출력하겠다 -->
								<td>${m.tel}</td>
								<td>${m.address}</td>
								<td><a href="delete?email={${m.email}">del</a></td> <!-- 데이터 보내는 작업 -->
								<!-- 쿼리스트링 : 사용자가 입력데이터를 url 주소에 포함하여 넘기는 것 -->
								<!-- 쿼리스트링의 시작은 주소 이후에 ? 뒤부터이다! -->
